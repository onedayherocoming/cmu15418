...
__global__ void upsweep_kernel(int *data, int N, int twod)
{
    ...
    if (index < N && index % (twod * 2) == 0)
    {
        data[index + twod * 2 - 1] += data[index + twod - 1];
    }
}
...
__global__ void downsweep_kernel(int *data, int N, int twod)
{
    ...
    if (index < N && index % (twod * 2) == 0)
    {
        ...
        data[index + twod - 1] = data[index + twod * 2 - 1];
        data[index + twod * 2 - 1] += t;
    }
}
...
__global__ void find_peak_kernel(int *data, int N, int *res)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < N)
    {
...
int find_peaks(int *device_input, int length, int *device_output)
{
    ...
    find_peak_kernel<<<blocks, threadsPerBlock>>>(device_input, length, device_result);
    int *host_indices = (int *)malloc(sizeof(int) * N);
    cudaMemcpy(host_indices, device_result, N * sizeof(int), cudaMemcpyDeviceToHost);
    int count = 0;
    for (int i = 0; i < N; i++)
    {
        if (host_indices[i] == 1)
    ...
    int *host_output = (int *)malloc(sizeof(int) * count);
    int cur = 0;
    for (int i = 0; i < N; i++)
    {
        if (host_indices[i] == 1)
    ...
    cudaMemcpy(device_output, host_output, count * sizeof(int), cudaMemcpyHostToDevice);
